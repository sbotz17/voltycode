...
class GeminiProvider(AIProvider):
    ...
    async def chat(self, request: ChatRequest) -> ChatResponse:
        ...
        content = "Gemini provider placeholder response"
        if request.messages:
            content = f"Gemini: {request.messages[-1].content}"
        return ChatResponse(
            provider=self.name,
    ...
    async def models(self) -> list[ModelInfo]:
        return [
            ModelInfo(id="gemini-1.5-flash", name="Gemini 1.5 Flash", provider=self.name),
            ModelInfo(id="gemini-1.5-pro", name="Gemini 1.5 Pro", provider=self.name),
        ]
